    ...
    <template slot="content">
        ...
        <el-row >
          <el-col :span="11" style="overflow: hidden; padding-top:40px">
           ...
           <ty-upload :action="action" v-model="this.form.appointmentFileId" :multiple="false" :length='1' :imgMaxSize="2" :imgSuffixs='["png","bmp","jpg", "jpeg"]'
                  resFormat="data" class="tyUpload" :width="100" :height="100" @change="imgChange" :disabled="false">
                  <div slot="tip">
      ...
      <hqit-dialog
         ...
         dialogType="form"
          width="68%"
        >
          <person-sel  :radiodata.sync="radiodata"></person-sel>
...
this.personDetail.fileIdList.push(this.src + item)
    ...
    getPersonDetail2(id) {
      getPerson2(id).then((res) => {
        ...
        let obj = {}
        obj.url = this.src+res.data.appointmentFileId
         this.$set(this.form, 'appointmentFileId',[obj ])

        // let arr = [res.data.appointmentFileId];
    ...
    config() {
      ...
      this.$refs.ruleForm.$refs.personIdFormItem[0].$emit(
        "el.form.change",
    ...
    imgChange(val) {
      ...
      if(val.res && val.res.id){
           let obj = {};
          obj.url = val.res.downloadUrl;
          obj.id =val.res.id;
          obj.name = val.res.name;
          this.$set(this.form, 'appointmentFileId', [obj])
        // this.form.appointmentFileId[0] = obj;
      }else{
           this.form.appointmentFileId=[]
    ...
    handleClose() {
      this.name = "";
      this.form.personId = "";
      this.$refs.ruleForm.$refs.personIdFormItem[0].$emit("el.form.change", null);
    },
    // //查看大图
    bigPic(list) {
      this.dialogPic = true;
