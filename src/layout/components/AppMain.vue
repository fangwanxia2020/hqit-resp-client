<template>
  <section class="app-main" :style="!this.$store.state.settings.tagsview?'min-height:calc(100vh - 70px)':''">
  ...
  computed: {
    ...
    cachedViews() {
      ...
      if (this.isInBaseSite) {
        ...
        if (this.user.loadedApp['/resp']) {
          const { childRoute } = this.user.loadedApp['/resp']
